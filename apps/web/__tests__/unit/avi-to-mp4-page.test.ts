...
const pageSource = readFileSync(
	join(process.cwd(), "app/(site)/tools/convert/avi-to-mp4/page.tsx"),
	"utf-8",
);

describe("AVI to MP4 page metadata", () => {
	...
	it("contains keywords array", () => {
		...
		expect(pageSource).toContain("avi to mp4 converter");
	});
});

describe("AVI to MP4 page structured data", () => {
...
describe("AVI to MP4 FAQ schema validity", () => {
	const faqs = [
		{
...
describe("AVI to MP4 HowTo schema validity", () => {
	const howToSteps = [
		{
	...
	it("assigns correct positions to each step", () => {
		const schema = createHowToSchema({
			name: "How to Convert AVI to MP4 Online",
			description: "Convert AVI to MP4 in your browser.",
			steps: howToSteps,
		});
